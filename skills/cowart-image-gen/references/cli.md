...
## Quick start
From the Cowart repo root, set a stable path to this local fork's CLI:

```
export IMAGE_GEN="skills/cowart-image-gen/scripts/image_gen.py"
```
...
- Do not modify `scripts/image_gen.py` unless the user explicitly asks for a CLI behavior change. If something is missing, document the gap before editing the copied CLI.
...
- Built-in-first transparent image workflow: `SKILL.md` and `skills/cowart-image-gen/scripts/remove_chroma_key.py`
